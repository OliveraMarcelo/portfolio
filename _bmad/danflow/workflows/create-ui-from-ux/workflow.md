...
9. **Probar `file://`, porque es como el usuario lo abre.** Un prototipo de varias pantallas
   se abre con doble clic, y ahí cambian dos cosas: no hay servidor, y **cada archivo es un
   origen opaco distinto**, así que `localStorage` no se comparte entre pantallas y el tema
   y el idioma se pierden en cada salto. Requisitos para que el prototipo aguante `file://`:

   - Estado (tema, idioma) propagado por **query string** en los enlaces internos,
     con `localStorage` como respaldo.
   - `@view-transition` inyectada por JS solo bajo http/https.
   - Ninguna dependencia de `fetch` en el runtime.

   Verificación sin servidor, con un clic real:

   ```bash
   # copiar la pantalla, inyectarle un click al enlace, y ver el DOM final
   google-chrome --headless=new --disable-gpu --dump-dom --virtual-time-budget=6000 \
     "file:///ruta/pantalla/__test_click.html" | grep -oE '<title>[^<]*</title>'
   ```

   Si el `<title>` es el del destino, la navegación ocurrió de verdad.
...
| **Con `file://` cada documento es un origen opaco distinto: `localStorage` NO se comparte.** El usuario elige tema o idioma, navega a otra pantalla y la preferencia se pierde — se percibe como "la navegación se rompe" | Propagar el estado por **query string** (`?theme=&lang=`), que sí viaja entre archivos; `localStorage` queda como respaldo para HTTP |
| Las cross-document view transitions exigen same-origin, así que en `file://` no aplican | No declarar `@view-transition` en el CSS: inyectarla desde JS solo si el protocolo es http/https |
| **Verificar solo por HTTP no alcanza**: el usuario suele abrir el prototipo con doble clic | Probar también `file://`. Con `google-chrome --headless=new --dump-dom --virtual-time-budget=5000 file://…` se puede inyectar un clic y confirmar la navegación real |
| Un fix "plausible" puede no ser la causa | Probar **con y sin** el cambio antes de atribuirle la corrección: si el síntoma no cambia, seguir buscando |
